...
class TestSearchWebDirect:
    async def test_missing_api_key_returns_none(self):
        with (
            patch("utils.web_search.BRAVE_SEARCH_API_KEY", ""),
            patch("utils.web_search.logger.error") as mock_log,
        ):
    ...
    async def test_sends_request_with_expected_headers_and_params(self):
        ...
        with (
            patch("utils.web_search.BRAVE_SEARCH_API_KEY", "test-key-123"),
    ...
    async def test_includes_search_lang_when_provided(self):
        ...
        assert result == []
        params = mock_client.return_value.__aenter__.return_value.get.call_args[1]["params"]
    ...
    async def test_non_2xx_response_returns_none_and_logs(self):
        ...
        assert result is None
        mock_log.assert_called_once_with("Brave Search API error: 401 Unauthorized")
    ...
    async def test_exception_during_request_returns_none_and_logs(self):
        ...
        assert result is None
        mock_log.assert_called_once_with("Brave Search API exception: Connection failed")

    async def test_successful_response_returns_formatted_results(self):
        mock_json = {
            "web": {
                "results": [
                    {"title": "Result A", "description": "Desc A", "url": "https://a.com"},
                    {"title": "Result B", "description": "Desc B", "url": "https://b.com"},
                    {"title": "Result C", "description": "Desc C", "url": "https://c.com"},
                ]
            }
        }
        mock_response = MagicMock(is_success=True)
        ...
        assert len(result) == 2
        assert result[0] == {"title": "Result A", "description": "Desc A", "url": "https://a.com"}
        assert result[1] == {"title": "Result B", "description": "Desc B", "url": "https://b.com"}

    async def test_honors_count_in_result_truncation(self):
        mock_json = {
            "web": {
                "results": [
                    {"title": f"Result {i}", "description": "", "url": ""} for i in range(10)
                ]
            }
        }
        mock_response = MagicMock(is_success=True)
